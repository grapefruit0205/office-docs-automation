...
function _조회수식(찾을셀, 반환열문자) {
  const 방식 = String(설정('조회함수') || 'XLOOKUP').toUpperCase();
  const 반 = '$' + 반환열문자 + ':$' + 반환열문자;
  if (방식 === 'INDEXMATCH') {
    return '=IFERROR(INDEX(' + SH.설비 + '!' + 반 + ',MATCH(' + 찾을셀 + ',' + SH.설비 + '!$A:$A,0)),"미등록")';
  }
  return '=IFERROR(XLOOKUP(' + 찾을셀 + ',' + SH.설비 + '!$A:$A,' + SH.설비 + '!' + 반 + '),"미등록")';
}
...
function 대장서식적용(sh, 시작행) {
  ...
  매핑.forEach(([이름, 반환열]) => {
    ...
    if (!c) return;
    const 수식들 = [];
    for (let r = 머리 + 1; r <= 마지막; r++) 수식들.push([_조회수식('$' + 태그열 + r, 반환열)]);
...
function _카운트(조건들) {
  return '=COUNTIFS(' + 조건들.map(([c, v]) => SH.점검 + '!$' + c + ':$' + c + ',' + v).join(',') + ')';
}
...
function 요약갱신() {
  ...
  const 설비행들 = 설비들.map((r, i) => {
    ...
    const 항목수식 = 항목목록.map((_, k) => {
      const 열 = _열문자(4 + k);
      return _카운트([[태그열, '$A' + 행], [항목열, 열 + '$1'], [판정열, '"이상"']]);
    });
    값.push(...항목수식);
    값.push('=SUM(D' + 행 + ':L' + 행 + ')');
    값.push('=ROUND(COUNTIF(' + SH.점검 + '!$' + 태그열 + ':$' + 태그열 + ',$A' + 행 + ')/' + 항목목록.length + ',0)');
  ...
  const 항목행들 = 항목목록.map((항목, i) => {
    ...
    return [
      항목,
      _카운트([[항목열, '$A' + 행], [판정열, '"이상"']]),
      _카운트([[항목열, '$A' + 행], [판정열, '"이상"'], [심각열, '"상"']]),
      _카운트([[항목열, '$A' + 행], [판정열, '"이상"'], [심각열, '"중"']]),
      _카운트([[항목열, '$A' + 행], [판정열, '"이상"'], [심각열, '"하"']]),
      _카운트([[항목열, '$A' + 행], [판정열, '"이상"'], [조치열, '"미조치"']]),
      '=IFERROR(TEXT(MAXIFS(' + SH.점검 + '!$B:$B,' + SH.점검 + '!$' + 항목열 + ':$' + 항목열 + ',$A' + 행 +
        ',' + SH.점검 + '!$' + 판정열 + ':$' + 판정열 + ',"이상"),"yyyy-mm-dd"),"—")',
    ];
  });
  항목탭.getRange(2, 1, 항목행들.length, 항목행들[0].length).setValues(항목행들);

  _로그('요약: 설비 ' + 설비행들.length + ' × 항목 ' + 항목목록.length + ' 수식, 항목별 ' + 항목행들.length + '행');
}
...
function 설치_샘플데이터() {
  ...
  설비탭.getRange(2, 1, 설비정의.length, 탭정의[0].헤더.length)
    .setValues(설비정의.map((d) => 탭정의[0].헤더.map((h) => d[h])));

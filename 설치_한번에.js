...
function 대장서식적용(sh, 시작행) {
  ...
  const 너비 = {
    점검ID: 145, 점검일: 92, 설비태그: 68, 설비명: 110, 구역: 70, 담당자: 65, 점검자: 65,
...
function 요약갱신() {
  ...
  const 설비행들 = 설비들.map((r, i) => {
    ...
    // 이상률 분모는 '그 설비에 해당 있는 항목'의 판정 건수다(측정값 '-'(해당 없음) 제외).
    // 보고서의 이상률과 같은 정의를 써야 두 화면의 숫자가 어긋나지 않는다.
    값.push('=IFERROR(ROUND(M' + 행 + '/COUNTIFS(' + SH.점검 + '!$' + 태그열 + '$2:$' + 태그열 + '$' + 마지막 +
      ',$A' + 행 + ',' + SH.점검 + '!$' + 측정열 + '$2:$' + 측정열 + '$' + 마지막 + ',"<>-")*100,1),0)');
    return 값;
  });
  if (설비행들.length) {
...
function _안내작성(ss) {
  ...
  const 줄 = [
    ...
    ['이상률은 그 설비에 해당 있는 항목만 분모로 씁니다(예: 압축기의 탱크 액위는 제외) — 보고서의 이상률과 같은 기준입니다.', '본문'],
...
function 설치_확인() {
  ...
  검사('발행이력', () => {
    ...
    const 계획 = _발행이력완료행('작업계획서').length;
    const 보고 = _발행이력완료행('점검결과보고서').length;
    _로그('      · 작업계획서 완료 ' + 계획 + '건 / 점검결과보고서 완료 ' + 보고 + '건 (전체 ' + 전체 + '행)');
    if (계획 > 20) return '작업계획서 완료가 ' + 계획 + '건(20건 초과) — 설치_전체를 실행하면 중복이 정리됩니다';
    if (보고 > 20) return '점검결과보고서 완료가 ' + 보고 + '건(20건 초과) — 설치_전체를 실행하면 중복이 정리됩니다';
    return 전체 ? true : '완료 건이 없습니다 — 양식_일괄생성을 실행하세요';
  });
